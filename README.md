...
<details>
  <summary>📑 <strong>Table of Contents / 目次</strong></summary>

- [概要](#-overview--概要)
- [主な特徴](#-features--主な特徴)
- [必要要件](#-prerequisites--必要環境)
- [インストール](#-installation--インストール)
- [使い方](#-quick-start--使い方)
  - [1. 初期設定（Settings）](#1-初期設定settings)
  - [2. 動画の自動クリッピング（Select Video）](#2-動画の切り抜きselect-video)
- [技術スタック](#-tech-stack--技術構成)
- [ライセンス](#-license--ライセンス)
...
- 🎯 **高精度な画像認識**: 指定したUIやテキスト（「DUEL 1」「SLASH」「ROUND 1」等）を検出し、試合区間を自動特定。
- ⚡ **高速なROI（特定領域）マッチング**: 画面全体ではなく特定エリアのみを判定対象にすることで、CPU負荷を最小化しながら高速検出を実現。
- 🏆 **勝敗自動判定対応**: 自プレイヤーの勝利・敗北テンプレートを登録することで、勝敗結果に応じた自動判定が可能。
- 🖥️ **軽量かつモダンなGUI**: `egui` によるネイティブ並みに軽快で直感的な操作インターフェース。
- 🎬 **FFmpeg 連携**: 高速かつ安定した動画デコード・トリミング処理。
- ⚙️ **柔軟なパラメータ調整**: 一致判定しきい値、探索フレームステップ数、前後のマージン（オフセット）を自由にカスタマイズ可能。設定は自動保存。
...
設定後、下部の **「Save」** ボタンをクリックして保存します（次回以降も自動で保持されます）。
